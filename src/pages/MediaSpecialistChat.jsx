...
export default function MediaSpecialistChat() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load businesses for this employee
        const businessList = await base44.entities.Business.list();
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!messageText.trim() || !selectedBusiness) return;
        ...
        <div className="mb-6">
          <div className="inline-block mb-3">
            <div className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm">
              ...
              <span className="text-xs text-[#6b7055] font-medium uppercase tracking-wider">Client Messages</span>
            </div>
          </div>
          ...
          <p className="text-[#6b7055] text-lg">
            Direct messaging with your business clients
          </p>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-[#6b7055] text-lg max-w-md">
                Once you add businesses, you'll be able to message their clients here.
              </p>
            </div>
          </div>
        ) : (
          <>
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-[#e8e6de]/50 mb-4">
              <select
                ...
                className="w-full px-4 py-2 rounded-lg border-[#e8e6de] bg-white text-[#2d3319] font-medium"
              >
            ...
            {messages.length === 0 ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  <MessageCircle className="w-16 h-16 mx-auto text-[#d4d2c8] mb-6" />
                  <h3 className="text-2xl font-bold text-[#2d3319] mb-3">No messages yet</h3>
                  <p className="text-[#6b7055] text-lg">
              ...
              <div className="flex-1 bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-lg border border-[#e8e6de]/50 overflow-y-auto mb-4">
                <div className="space-y-4">
                  {messages.map(msg => (
                    <div key={msg.id} className={`flex ${msg.isFromEmployee ? 'justify-end' : 'justify-start'}`}>
